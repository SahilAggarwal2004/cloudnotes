  ...
  async function submit(event) {
    ...
    if (stage === "otp")
      fetchApi({
        ...
        onError: (error) => {
          if (error === "OTP already sent!") setStage("reset");
        },
      });
    else
      fetchApi({
          ...
          <form className="mt-8 space-y-6" onSubmit={submit}>
            <div className="-space-y-px rounded-md shadow-xs">
              <input
                ...
                autoComplete="email"
                required
                className={`relative block w-full appearance-none rounded-md border border-gray-300 px-3 py-2 text-gray-900 placeholder-gray-500 ${stage === "otp" ? "" : "rounded-b-none"} focus:z-10 focus:border-purple-600 focus:outline-hidden focus:ring-purple-600 sm:text-sm`}
            ...
            <button type="submit" className="button-animation relative flex w-full justify-center rounded-md border px-4 py-2 text-sm font-medium">
              {stage === "otp" ? "Send OTP" : "Reset password"}
            </button>
          </form>
        </div>
      </div>
